...
(function (window, undefined) {
  ...
  $j(document).ready(function () {
    ...
    $(document.body).observe(
      'cel_yuiOverlay:afterShowDialog_General',
  ...
  if (typeof window.CELEMENTS.presentation.SlideShow === 'undefined') {
    window.CELEMENTS.presentation.SlideShow = Class.create({
      ...
      initialize: function (containerId) {
        ...
        _me._navObj = new CELEMENTS.presentation.Navigation(
          _me._preloadSlide.bind(_me),
          _me._showSlide.bind(_me),
          _me._waitingLoad.bind(_me),
        );
        ...
        _me._registerOnOpenOverlayCheckerBind =
          _me._registerOnOpenOverlayChecker.bind(_me);
        _me._imgLoadedResizeAndCenterSlideBind =
          _me._imgLoadedResizeAndCenterSlide.bind(_me);
      ...
      _registerOnOpenOverlayChecker: function (event) {
        ...
        const checkRegisterEvent = $(document.body).fire(
          'cel_slideShow:shouldRegister',
          {
            origEvent: event,
            slideShow: _me,
      ...
      _waitingLoad: function (waitingObj) {
        ...
        if (waitingObj.waitingMode == 'next') {
          ...
          _me._htmlContainer.fire(
            'cel_slideShow:waitingNextLoad-' + waitingObj.waitingState,
            waitingObj,
          );
          ...
          _me._htmlContainer.fire(
            'cel_slideShow:waitingPrevLoad-' + waitingObj.waitingState,
            waitingObj,
          );
        } else {
          if (waitingObj.waitingState == 'start') {
          ...
          _me._htmlContainer.fire(
            'cel_slideShow:waitingLoad-' + waitingObj.waitingState,
            waitingObj,
          );
        }
      },
      ...
      _addWrapperElements: function (slideContent) {
        const slideWrapperElem = new Element('div')
          .addClassName('cel_slideShow_slideWrapper')
          .setStyle({
            position: 'relative',
          })
          .update(slideContent);
        const slideRootElem = new Element('div')
          .addClassName('cel_slideShow_slideRoot')
          .setStyle({
            position: 'absolute',
          })
      ...
      register: function () {
        ...
        if (_me._htmlContainer) {
          ...
          _me._htmlContainer.observe(
            'cel_yuiOverlay:afterSlideInsert',
          ...
          _me._htmlContainer.observe(
            'cel_slideShow:preloadContentFinished',
            _me._preloadSlideImages.bind(_me),
          );
          $(document.body).observe(
            'cel_yuiOverlay:hideEvent',
            _me._resetContainerElem.bind(_me),
          );
        } else {
          if (
            ...
            console.warn(
              'register slideshow failed. Reason: container with id "' +
      ...
      _registerDynamicProviderAfterContentChanged: function () {
        ...
        $(document.body).fire(
          'cel_slideShow:registerAfterContentChanged',
          _me._htmlContainer,
        );
      },
      ...
      _resetContainerElem: function (event) {
        ...
        if (_me._debug) {
          console.log(
            '_resetContainerElem: for htmlElem ',
          ...
          onSuccess: function (transport) {
            if (transport.responseText.isJSON()) {
              ...
              if (
                ...
                console.error(
                  'loadAndAddMainSlides returns no Json: ',
            ...
            if (transport.responseText.isJSON()) {
              ...
              if (typeof startAtIndexOrName === 'string') {
                if (startAtIndexOrName === '!RANDOM!') {
                  startAtIndex = Math.floor(
                    Math.random() * (jsonObj.length - 1),
                  );
                } else {
                  startAtIndex = jsonObj.indexOf(
                    spaceName + '.' + startAtIndexOrName,
                  );
                }
              }
              ...
              if (
                ...
                console.error(
                  'loadMainSlides returns no Json: ',
                  transport.responseText,
                );
              }
            }
          ...
          onSuccess: function (transport) {
            if (transport.responseText.isJSON()) {
              ...
              if (
                ...
                console.error(
                  'loadAndAddSubSlides returns no Json: ',
                  transport.responseText,
                );
              }
            }
      ...
      _updateSlideCounter: function (newSlideWrapperElem) {
        ...
        if (_me._debug) {
          console.debug(
            '_updateSlideCounter: ',
            numSlides,
            _me._htmlContainer.select('.celPresSlideShow_countSlideNum'),
          );
        }
        _me._htmlContainer
          .select('.celPresSlideShow_countSlideNum')
          .each(function (countSlideElem) {
        ...
        _me._htmlContainer
          .select('.celPresSlideShow_currentSlideNum')
          .each(function (currentSlideElem) {
      ...
      _gotoSlideClickHandler: function (event) {
        ...
        if (_me.gotoSlide(fullName)) {
          ...
          const nonSlideClickEvent = _me._htmlContainer.fire(
            'cel_slideShow:nonSlideClick',
            {
              theSlideShow: _me,
              fullName: fullName,
              clickEvent: event,
          ...
          if (!nonSlideClickEvent.stopped) {
            if (
              ...
              console.error(
                '_gotoSlideClickHandler failed. Index not found for: ',
                fullName,
      ...
      _registerDirectLinks: function () {
        const _me = this;
        _me._htmlContainer
          .select('.celPresSlideShow_navigation')
          .each(function (navElem) {
            ...
            if (
              ...
              if (!beforeRedirEvent.stopped) {
                if (!_me.gotoSlide(jsonObj.redirectFN)) {
                  _me._htmlContainer.fire(
                    'cel_slideShow:preloadContentRedirectFailed',
                    {
                      theSlideShow: _me,
                      slideFN: slideFN,
                      jsonObj: jsonObj,
      ...
      _preloadSlideImages: function (event) {
        ...
        if (!_me._preloadSlideImagesHash.get(preLoadSlide.slideFN)) {
          const preloadElem = new Element('div').update(
            preLoadSlide.responseText,
          );
      ...
      _centerCurrentSlide: function (slideWrapperIn) {
        ...
        if (_me._debug) {
          console.debug(
            '_centerCurrentSlide slideRoot dim: ',
            _me._htmlContainerId,
            slideOuterWidth,
            slideOuterHeight,
            slideRoot.getStyle('position'),
            slideRoot,
          );
        }
        ...
        if (_me._debug) {
          console.debug(
            '_centerCurrentSlide parentDiv dim: ',
            _me._htmlContainerId,
            parentWidth,
            parentHeight,
            parentDiv.scrollWidth,
      ...
      _imgLoadedResizeAndCenterSlide: function (
        imgElem,
        slideWrapper,
        callbackFN,
        event,
        ...
        if (_me._debug) {
          console.debug(
            '_imgLoadedResizeAndCenterSlide: preloading finished for ',
            imgElem.inspect(),
          );
        }
        ...
        if (_me._preloadingImageQueue.length == 0) {
          if (_me._debug) {
            console.log(
              '_imgLoadedResizeAndCenterSlide before _resizeAndCenterSlidefor ',
              slideWrapper,
            );
            slideWrapper.select('img').each(function (imgElem) {
              console.debug(
                '_imgLoadedResizeAndCenterSlide: img loaded ',
                imgElem.inspect(),
                imgElem.complete,
              );
            });
      ...
      _resizeAndCenterSlide: function (slideWrapper) {
        ...
        _me._htmlContainer.fire(
          'cel_slideShow:beforeResizeAndCenterSlide',
          _me,
        );
        if (_me._resizeSlide) {
          const resizeSlideEvent = _me._htmlContainer.fire(
            'cel_slideShow:resizeSlide',
            _me,
          );
          if (!resizeSlideEvent.stopped) {
            ...
            console.log(
              '_resizeAndCenterSlide: resizeSlideEvent.stopped ',
              slideWrapper,
            );
          }
        }
        if (_me._centerSlide) {
          const centerSlideEvent = _me._htmlContainer.fire(
            'cel_slideShow:centerSlide',
            _me,
          );
          if (!centerSlideEvent.stopped) {
            ...
            console.log(
              '_resizeAndCenterSlide: centerSlideEvent.stopped ',
              slideWrapper,
            );
          }
        }
      ...
      _resizeCurrentSlide: function (slideWrapperIn) {
        ...
        if (_me._autoresize && zoomFactorObj.zoomFactor <= 1) {
          ...
          const resizeEvent = _me._htmlContainer.fire(
            'cel_slideShow:resizeSlideContent',
            eventMemo,
          );
          if (!resizeEvent.stopped) {
            slideWrapper.setStyle(
              _me.getZoomStyles(
                zoomFactorObj.zoomFactor,
                zoomFactorObj.oldWidth,
                zoomFactorObj.oldHeight,
              ),
            );
          }
          ...
          if (parentDiv.hasClassName('cel_slideShow_slideRoot')) {
            if (_me._debug) {
              console.debug(
                '_resizeCurrentSlide: setting rootElem new size ',
                _me._htmlContainerId,
                zoomFactorObj.newWidth,
                zoomFactorObj.newHeight,
              );
            }
            parentDiv.setStyle({
          ...
          if (zoomFactorObj.zoomFactor > 1) {
            ...
            console.log('resize skipped.', {
              ...
              zoomFactorObj: zoomFactorObj,
            });
          }
          //set sizes without zoom too. Important for centering
          if (_me._debug) {
            console.debug(
              '_resizeCurrentSlide: setting wrapper size ',
              zoomFactorObj.oldWidth,
              zoomFactorObj.oldHeight,
            );
          }
          ...
          if (parentDiv.hasClassName('cel_slideShow_slideRoot')) {
            if (_me._debug) {
              console.debug(
                '_resizeCurrentSlide: setting rootElem old size ',
                _me._htmlContainerId,
                zoomFactorObj.oldWidth,
                zoomFactorObj.oldHeight,
              );
      ...
      _showSlide: function (slideContent) {
        ...
        const slideWrapperElem = slideRootElem.down(
          '.cel_slideShow_slideWrapper',
        );
        if (_me._debug) {
          console.debug(
            '_showSlide: before cel_yuiOverlay:beforeSlideInsert ',
            _me._htmlContainer,
          );
        }
        ...
        if (_me._debug) {
          console.debug(
            '_showSlide: before cel_yuiOverlay:afterSlideInsert event ',
            _me.getHtmlContainer(),
          );
        }
        ...
        const resizeAndCenterSlideEvent = _me._htmlContainer.fire(
          'cel_slideShow:resizeAndCenterSlide',
          _me,
        );
        if (!resizeAndCenterSlideEvent.stopped) {
          _me._preloadImagesAndResizeCenterSlide(
            slideWrapperElem,
      ...
      _preloadImagesAndResizeCenterSlide: function (
        slideWrapperElem,
        callbackFN,
      ) {
        const _me = this;
        ...
        slideWrapperElem.select('img').each(function (imgElem) {
          if (!imgElem.complete) {
            ...
            if (_me._debug) {
              console.debug(
                '_preloadImagesAndResizeCenterSlide: preloading started for ',
                imgElem.inspect(),
              );
            }
          } else if (_me._debug) {
            console.log(
              '_preloadImagesAndResizeCenterSlide: NO preloading for ',
              imgElem.inspect(),
            );
          }
        });
        if (_me._preloadingImageQueue.size() == 0) {
          if (_me._debug) {
            console.log(
              '_preloadImagesAndResizeCenterSlide before _resizeAndCenterSlide' +
                'for ',
              slideWrapperElem,
            );
            slideWrapperElem.select('img').each(function (imgElem) {
              console.debug(
                '_preloadImagesAndResizeCenterSlide: img loaded ',
                imgElem.inspect(),
                imgElem.complete,
              );
            });
      ...
      _showSlideAfterPreloadingImg: function () {
        ...
        _me._htmlContainer.stopObserving(
        ...
        const contentChangeEvent = _me._htmlContainer.fire(
          'cel_yuiOverlay:changeContent',
          {
            slideShowObj: _me,
            slides: slides,
          },
