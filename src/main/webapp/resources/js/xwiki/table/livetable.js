(function () {
  ...
  if (window.celExecOnceAfterMessagesLoaded) {
    window.celExecOnceAfterMessagesLoaded((celMessages) => (translations = celMessages.livetable));
  } else {
  ...
  XWiki.widgets.LiveTable = Class.create({
    ...
    initialize: function (url, domNodeName, handler, options) {
      ...
      if (typeof this.pageSizeNodes != 'undefined') {
        this.pageSizer = new LiveTablePageSizer(
          this,
          this.pageSizeNodes,
          options.pageSizeBounds,
          this.limit,
        );
      }
      ...
      if (typeof this.paginationNodes != 'undefined') {
        this.paginator = new LiveTablePagination(
          this,
          this.paginationNodes,
          options.maxPages || 10,
        );
      ...
      this.observeSortableColumns(
        this.permalinks.getSortColumn(),
        this.permalinks.getSortDirection(),
      );
      ...
      $(this.domNodeName)
        ...
        .each(function (el) {
          ...
          if (colname == column) {
            ...
            if (!el.hasClassName('desc') && !el.hasClassName('asc')) {
              // no order set in the HTML. Force desc
              el.addClassName('desc');
            }
          }
          Event.observe(el, 'click', function (event) {
            ...
            if (elem.hasClassName('selected')) {
              // Sort column already selected. Change direction
              ...
              // sort column was not selected, do not change direction, just column
  ...
  var LiveTableHash = Class.create({
     ...
     */
    getLimit: function () {
      return this.getIntParam('l');
    },
    getPage: function () {
      return this.getIntParam('p');
    },
    getSortColumn: function () {
      return this.getParam('s');
    },
    getSortDirection: function () {
      return this.getParam('d');
    },
  ...
  var LiveTablePagination = Class.create({
    initialize: function (table, domNodes, max) {
      ...
      this.max = max;
      $(this.table.domNodeName)
        .select('.prevPagination')
        .invoke('observe', 'click', this.gotoPrevPage.bind(this));
      $(this.table.domNodeName)
        .select('.nextPagination')
        .invoke('observe', 'click', this.gotoNextPage.bind(this));
    },
  ...
  var LiveTableFilter = Class.create({
    ...
    attachEventHandlers: function () {
      ...
      document.observe(
        'xwiki:livetable:' + this.table.domNodeName + ':filtersChanged',
