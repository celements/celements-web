...
function init_sortnfilter() {
    ...
    for (var A = 0;
        ...
        A++) {
        var B = C[A];
        if (((" " + B.className + " ").indexOf("sortable") != -1) && (B.id)) {
            ts_makeSortable(B)
        }
    }
    ...
    for (var A = 0;
        ...
        A++) {
        var B = C[A];
        if (((" " + B.className + " ").indexOf("filterable") != -1) && (B.id)) {
            setFilterGrid(B.id, (getHeaderRow(B)))
        }
    }
    ...
    while (!E) {
        E = ts_getInnerText(P.rows[K].cells[D]);
        K++
    } if (!E) {
        E = ""
    ...
    if (N.getAttribute("sortdir") == "down") {
        L = '<img border="0" src="' + image_path + image_up + '" alt="&uarr;"/>';
        Q.reverse();
        N.setAttribute("sortdir", "up")
    } else {
        L = '<img border="0" src="' + image_path + image_down + '" alt="&darr;"/>';
        ...
        O < Q.length;
        O++) {
        if (Q[O].className && (Q[O].className.indexOf("sortBottom") != -1)) {
            P.tBodies[0].appendChild(Q[O])
        }
    } for (var O = 0;
        O < J.length;
        O++) {
        P.tBodies[0].appendChild(J[O])
    ...
    for (var Q = 0;
        Q < SearchFlt.length;
        ++Q) {
        O.push((document.getElementById(SearchFlt[Q]).value).toLowerCase())
    } var F = getStartRow(J);
    ...
    for (var N = F;
        ...
        if (K == M) {
            ...
            for (var P = 0;
                P < K;
                P++) {
                var E = getCellText(B[P]).toLowerCase();
                S.push(E);
                if (O[P] != "") {
                    var R = parseFloat(E);
                    if (/<=/.test(O[P]) && !isNaN(R)) {
...
} function getHeaderRow(B) {
    for (var A = 0;
        A < B.rows.length - 1;
        A++) {
        if (B.rows[A].className.indexOf("sortHeader") > -1) {
            return A
        }
    } return 0
} function getSortBottomRow(B) {
    for (var A = 0;
        A < B.rows.length;
        A++) {
        if (B.rows[A].className.indexOf("sortBottom") > -1) {
            return A
        }
    } return null
} function ts_getInnerText(D) {
    if (typeof D == "string") {
        return D
    } if (typeof D == "undefined") {
        return ""
    } if (typeof D == "object" && D.tagName.toLowerCase() == "img") {
        return D.alt
    } if (D.innerText) {
        return D.innerText
    } var E = "";
    ...
    for (var B = 0;
        ...
        B++) {
        switch (C[B].nodeType) {
            case 1: E += ts_getInnerText(C[B]);
                break;
            case 3: E += C[B].nodeValue;
                break
        }
    } return E
} function ts_makeSortable(D) {
    var E;
    if (D.rows && D.rows.length > 0) {
        E = D.rows[getHeaderRow(D)]
    } if (!E) {
        return
    } for (var C = 0;
        C < E.cells.length;
        ...
        if (B.className != "unsortable" && B.className.indexOf("unsortable") == -1) {
            B.innerHTML = '<a href="#" class="sortHeader" onclick="ts_resortTable(this);return false;">' + A + '<span class="sortarrow"><img border="0" src="' + image_path + image_none + '" alt="&darr;"/></span></a>'
        }
    } alternate(D)
} function alternate(F) {
    if (F.className.indexOf("doOddEven") > -1) {
        var G = 1;
        ...
        for (var C = 0;
            C < D.length;
            C++) {
            var E = D[C].getElementsByTagName("tr");
            for (var B = getHeaderRow(F) + 1;
                B < A;
                B++) {
                if (E[B].style.display == "") {
                    G++;
                    swapOddEven(E[B], G)
                }
            }
...
} function replace(D, C, A) {
    var B = D.indexOf(C);
    var E = "";
    if (B == -1) {
        return D
    } E += D.substring(0, B) + A;
    if (B + C.length < D.length) {
        E += replace(D.substring(B + C.length, D.length), C, A)
    } return E
} function setFilterGrid(G) {
    var E = document.getElementById(G);
    var F, D;
    if (E != null && E.nodeName.toLowerCase() == "table") {
        TblId.push(G);
        if (arguments.length > 1) {
            for (var B = 0;
                B < arguments.length;
                B++) {
                var A = typeof arguments[B];
                switch (A.toLowerCase()) {
                    case "number": F = arguments[B];
                        break;
    ...
    if (O) {
        ...
        I < G;
        I++) {
        var E = M.insertCell(I);
        var H = A.cells[I];
        I == G - 1 && Q == true ? K = "flt_s" : K = "flt";
        if ((J == undefined || J["col_" + I] == undefined || J["col_" + I] == "none") && H.className.indexOf("selectFilter") == -1) {
            var N = document.createElement("input");
            N.setAttribute("id", "flt" + I + "_" + C);
            N.setAttribute("size", "5");
            if (H.className.indexOf("noFilter") == -1) {
            ...
            E.appendChild(N);
            if (L) {
                N.onkeypress = DetectKey
            }
        } else {
            if (H.className.indexOf("selectFilter") != -1) {
            ...
            D.className = "btnflt";
            E.appendChild(D);
            (!O) ? D.onclick = function () {
                Filter(C)
            } : D.onclick = J.mod_filter_fn
        }
    }
} function PopulateOptions(B, J, I) {
    ...
    for (var D = K;
        D < N.length;
        D++) {
        var L = getChildElms(N[D]).childNodes;
        var C = L.length;
        if (C == I) {
            for (var E = 0;
                E < C;
                E++) {
                if (J == E) {
                    var G = getCellText(L[E]);
                    if (F.toString().toUpperCase().search(G.toUpperCase()) == -1) {
                        H++;
                        F.push(G);
...
} function getCellsNb(E, C) {
    var A = document.getElementById(E);
    var B;
    if (C == undefined) {
        B = A.getElementsByTagName("tr")[0]
    } else {
        B = A.getElementsByTagName("tr")[C]
    ...
    for (var A = 0;
        A < D.length;
        A++) {
        SearchFlt.push(D[A].firstChild.getAttribute("id"))
    }
} function getStartRow(C) {
    var B;
    for (var A in TblId) {
        if (TblId[A] == C) {
            B = StartRow[A]
    ...
    if (A) {
        ...
        if (D == "13") {
            ...
            for (var C in ModFn) {
                ModFnId[C] == B ? F = true : F = false
            } (F) ? ModFn[C].call() : Filter(B)
        }
    }
} function swapOddEven(B, A) {
    if ((A % 2) == 0) {
    ...
    var D, E, B;
    if (F.length == 10) {
        D = F.substr(6, 4) + F.substr(3, 2) + F.substr(0, 2)
    } else {
        E = F.substr(6, 2);
        if (parseInt(E) < 50) {
            E = "20" + E
        ...
        B = G.substr(6, 4) + G.substr(3, 2) + G.substr(0, 2)
    } else {
        E = G.substr(6, 2);
        if (parseInt(E) < 50) {
            E = "20" + E
...
} function ts_sort_caseinsensitive(B, A) {
    var C = ts_getInnerText(B.cells[SORT_COLUMN_INDEX]).toLowerCase();
...
} function ts_sort_default(B, A) {
    var C = ts_getInnerText(B.cells[SORT_COLUMN_INDEX]);
