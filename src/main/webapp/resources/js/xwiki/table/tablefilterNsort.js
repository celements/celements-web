...
function ts_resortTable(R) {
  ...
  for (var M = F; M < P.rows.length; M++) {
    J[J.length] = P.rows[M];
  }
  for (var M = getHeaderRow(P) + 1; M < P.rows.length; M++) {
    Q[Q.length] = P.rows[M];
  }
  Q.sort(C);
...
function Filter(J) {
  ...
  var M = getCellsNb(J);
  var L = 1;
  for (var Q = 0; Q < SearchFlt.length; ++Q) {
...
function getHeaderRow(B) {
  for (var A = 0; A < B.rows.length - 1; A++) {
  ...
  for (var C = 0; C < E.cells.length; C++) {
    ...
    if (
      ...
      B.innerHTML =
        ...
        A +
        '<span class="sortarrow"><img border="0" src="' +
        image_path +
        image_none +
        '" alt="&darr;"/></span></a>';
    }
...
function alternate(F) {
  if (F.className.indexOf('doOddEven') > -1) {
    ...
    for (var C = 0; C < D.length; C++) {
      ...
      for (var B = getHeaderRow(F) + 1; B < A; B++) {
