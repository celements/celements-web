...
XWiki.editors.FullScreenEditing = Class.create({
  ...
  initialize: function () {
    if (window.celExecOnceAfterMessagesLoaded) {
      window.celExecOnceAfterMessagesLoaded(
        celMessages => this.translations = celMessages.fullscreenEdit);
    } else {
      console.warn('celExecOnceAfterMessagesLoaded not available!');
    }
    this.buttons = $(document.body).down(".bottombuttons");
    if (!this.buttons) {
      this.buttons = new Element("div", { class: "bottombuttons" }).update(
    ...
    $$(".xRichTextEditor").each(
      function (B) {
        this.addBehavior(B);
      }.bind(this),
    );
    this.addWysiwyg20Listener();
    this.maximizedReference = $(document.body).down(
      "input[name='x-maximized']",
    );
    if (this.maximizedReference && this.maximizedReference.value != "") {
      var A = $$(this.maximizedReference.value);
      if (A && A.length > 0) {
        this.makeFullScreen(A[0]);
      }
    }
  },
  addBehavior: function (A) {
    if (this.isWysiwyg20Content(A)) {
      this.addWysiwyg20ContentButton(A);
    } else {
      if (this.isWysiwyg10Content(A)) {
        this.addWysiwyg10ContentButton(A);
      } else {
        if (this.isWikiContent(A)) {
          this.addWikiContentButton(A);
        } else {
          if (this.isWysiwyg20Field(A)) {
            this.addWysiwyg20FieldButton(A);
          } else {
            if (this.isWikiField(A)) {
              this.addWikiFieldButton(A);
            } else {
              if (this.isWysiwyg10Field(A)) {
                this.addWysiwyg10FieldButton(A);
              }
            }
          }
        }
  ...
  addWysiwyg20Listener: function () {
    document.observe(
      ...
      this.wysiwyg20Created.bindAsEventListener(this),
    );
  },
  wysiwyg20Created: function (B, A) {},
  isWikiContent: function (A) {
    return A.name == "content" && A.visible();
  },
  isWysiwyg10Content: function (A) {
    return (
      A.name == "content" &&
      (Prototype.Browser.IE
        ? A.previous(".mceEditorContainer")
        : A.next(".mceEditorContainer"))
    );
  },
  isWysiwyg20Content: function (A) {
    return (
      A.hasClassName("xRichTextEditor") && A.up("div[id^=content_container]")
    );
  },
  isWikiField: function (A) {
    return A.visible();
  },
  isWysiwyg10Field: function (A) {
    return (
      !A.visible() &&
      A.name != "content" &&
      (Prototype.Browser.IE
  ...
  isWysiwyg20Field: function (A) {
    return (
      A.hasClassName("xRichTextEditor") && !A.up("div[id^=content_container]")
    );
  },
  addWikiContentButton: function (A) {
    A._toolbar = $(document.body).down(".leftmenu2");
    if (A._toolbar) {
      A._toolbar.insert({ top: this.createOpenButton(A) });
    } else {
      this.addWikiFieldButton(A);
    }
  },
  addWysiwyg10ContentButton: function (E) {
    var B = Prototype.Browser.IE
      ? E.previous(".mceEditorContainer")
      : E.next(".mceEditorContainer");
    if (!B) {
      return false;
    }
    ...
    A.insert(
      new Element("img", {
        ...
        height: 15,
        width: 1,
        src: D.down("img.mceSeparatorLine").src,
      }),
    );
  ...
  addWysiwyg20ContentButton: function (B) {
    var A = B.down(".gwt-MenuBar");
    if (!A) {
      if (!B._x_fullScreenLoader) {
        B._x_fullScreenLoader_iterations = 0;
        B._x_fullScreenLoader = new PeriodicalExecuter(
          function (C) {
            if (C._x_fullScreenLoader_iteration > 100) {
              C._x_fullScreenLoader.stop();
    ...
    B._toolbar = A;
    if (B._x_fullScreenLoader) {
      B._x_fullScreenLoader.stop();
      B._x_fullScreenLoader = false;
    }
    return true;
  },
  addWikiFieldButton: function (A) {
    Element.insert(A, { before: this.createOpenLink(A) });
  },
  addWysiwyg10FieldButton: function (A) {
    this.addWysiwyg10ContentButton(A);
  },
  addWysiwyg20FieldButton: function (A) {
    this.addWysiwyg20ContentButton(A);
  },
  ...
  createCloseButtons: function () {
    ...
    this.closeButton.observe(
      "mousedown",
      this.preventDrag.bindAsEventListener(this),
    );
    this.closeButton.hide();
    this.actionCloseButton = new Element("input", {
      type: "button",
      class: "button",
      value: this.translations.exitFullScreen,
    });
    ...
    this.buttons
      .down(".buttons")
      .insert({ top: this.actionCloseButtonWrapper });
  },
  makeFullScreen: function (A) {
    if (this.maximizedReference) {
      if (A.id) {
        this.maximizedReference.value = A.tagName + "[id='" + A.id + "']";
      } else {
        if (A.name) {
          this.maximizedReference.value = A.tagName + "[name='" + A.name + "']";
        } else {
          if (A.className) {
            this.maximizedReference.value = A.tagName + "." + A.className;
          }
        }
      }
    }
    ...
    if (A.hasClassName("xRichTextEditor")) {
      var D = A.down(".gwt-RichTextArea");
      D._originalStyle = { width: D.style.width, height: D.style.height };
    } else {
      if (A.hasClassName("mceEditorContainer")) {
        var D = A.down(".mceEditorIframe");
        D._originalStyle = { width: D.style.width, height: D.style.height };
    ...
    if (A._toolbar) {
      if (A._toolbar.hasClassName("leftmenu2")) {
        E.insert({ top: A._toolbar.replace(this.toolbarPlaceholder) });
      }
    ...
    while (B != document.body) {
      ...
      B.siblings().each(function (F) {
        F._originalDisplay = F.style.display;
    ...
    if (A._toolbar) {
      A._toolbar.viewportOffset();
    }
  },
  closeFullScreen: function () {
    var A = this.maximized;
    this.closeButton.hide();
    this.actionCloseButtonWrapper.hide();
    ...
    while (B != document.body) {
      ...
      B.siblings().each(function (E) {
        E.style.display = E._originalDisplay;
      });
      B = B.up();
    }
    ...
    if (A._toolbar) {
      if (A._toolbar.hasClassName("leftmenu2")) {
        this.toolbarPlaceholder.replace(A._toolbar);
      }
  ...
  resizeTextArea: function (B) {
    ...
    if (B.hasClassName("xRichTextEditor")) {
      B.down(".gwt-RichTextArea").setStyle({
        width: C + "px",
        height:
          A -
          B.down(".xToolbar").getHeight() -
          B.down(".gwt-MenuBar").getHeight() +
          "px",
      });
    } else {
      if (B.hasClassName("mceEditorContainer")) {
        B.down(".mceEditorIframe").setStyle({
          width: C + "px",
          height: A - B._toolbar.getHeight() + "px",
        });
        B.down(".mceEditorSource").setStyle({
          width: C + "px",
          height: A - B._toolbar.getHeight() + "px",
        });
      }
