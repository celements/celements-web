$j(document).ready(function () {
  ...
  $$("#xwikiobjects a.edit").each(function (B) {
    B.observe(
      "click",
      function (C) {
        B.blur();
  ...
  $$("#xwikiobjects .xclass-title").each(function (B) {
    B.observe(
      "click",
      function (C) {
        B.up().toggleClassName("collapsed");
      }.bindAsEventListener(),
    );
  });
  $$("#xwikiobjects .xclass").each(function (B) {
    B.addClassName("collapsable");
  });
  var A = $$(".xproperty").size();
  $$(".xproperty").each(function (B) {
    B.addClassName("collapsable");
    if (A > 1) {
  ...
  $$("#xwikiobjects .xobject").each(function (B) {
    B.addClassName("collapsable");
    if (A > 1) {
      B.toggleClassName("collapsed");
    }
  });
  $$("#xwikiobjects .xobject-title").each(function (B) {
    B.observe(
      "click",
      function (C) {
        B.up().toggleClassName("collapsed");
  ...
  initialize: function () {
    ...
    $$(".xproperty-title").each(function (B) {
      var A = new Element("img", {
        src: "$xwiki.getSkinFile('icons/datamodel/move.png', true)",
        class: "move",
        alt: "move",
